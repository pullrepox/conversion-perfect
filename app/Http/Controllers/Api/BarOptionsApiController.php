...
class BarOptionsApiController extends Controller
{
	...
	public function __construct(BarsRepository $barsRepository, ApiRepository $apiRepository, ArListRepository $arListRepository)
	{
	...
	public function getAutoResponderLists(Request $request)
	{
		...
		if ($responder_id == 'conversion_perfect') {
			...
			$ar_lists = $this->arListRepo->getArListOptions(auth()->user()->id, $integration->id);
			if (!is_null($ar_lists) && !empty($ar_lists) && sizeof($ar_lists) > 0) {
				$re = [
					'result'  => 'success',
					'message' => $ar_lists
				];
				...
				if ($re['result'] == 'success') {
					$this->arListRepo->setArList(auth()->user()->id, $integration->id, $re['message']);
				}
			}
		}
