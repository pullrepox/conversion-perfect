...
new Vue({
    ...
    data: () => ({
        ...
        basic_model: {
            general: {redirect_mode: '301', tracker_password: '', notes: '', tags: '', erase_referral: null, forward_parameters: null},
    ...
    mounted() {
        let vm = this;
        this.create_edit = (window._tracker_opt_ary.create_edit || window._tracker_opt_ary.create_edit === 'true');
        ...
        Object.keys(this.tracker_option).forEach(function (item) {
            vm.tracker_option[item] = !window._tracker_opt_ary[item] ? false : (window._tracker_opt_ary[item] === 'true')
        });
        
        Object.keys(this.model).forEach(function (item) {
            if (window._tracker_opt_ary.model[item]) {
                vm.model[item] = window._tracker_opt_ary.model[item];
            }
        });
        ...
        if (this.tracker_option.geotargeting === 'false') {
            this.show_options.geo_include = true;
        }
        if (this.tracker_option.language === 'false') {
        ...
        this.initGeoTarget();
        this.initLangTarget();
        this.options_sub.split_weight = parseFloat(window._tracker_opt_ary.options_sub.split_weight);
    },
    methods: {
        initSelect2() {
        ...
        toggleOption(name) {
            this.tracker_option[name] = !this.tracker_option[name];
            if (this.del_option.name === 'split') {
                return;
            }
            axios.post(`/hide-option/${window._tracker_opt_ary.tracker_id}`, {
                option_key: name
            }).then((r) => {
            }).catch((e) => {
            });
        ...
        clearOption() {
            axios.post(`/clear-option/${window._tracker_opt_ary.tracker_id}`, {
                ...
                if (r.data.status === 'success') {
                    ...
                    this.tracker_option[this.del_option.key] = false;
        ...
        saveOption(key) {
            ...
            this.loading = true;
            axios.post(`/save-option/${window._tracker_opt_ary.tracker_id}`, save_data).then((r) => {
                this.loading = false;
                if (r.data.status === 'success') {
        ...
        addSplitRow() {
            ...
            axios.post(`/add-split-test/${window._tracker_opt_ary.tracker_id}`, this.model.split).then((r) => {
                if (r.data.status === 'success') {
        ...
        deleteSplitRow(ind) {
            this.showSaveBtn('split');
            axios.post(`/delete-split-test/${window._tracker_opt_ary.tracker_id}/${this.model.splits[ind].id}`).then((r) => {
                if (r.data.status === 'success') {
