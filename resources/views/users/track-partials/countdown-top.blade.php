<div style="height:20px; width:100%; text-align:center;
    ...
    @if ($bar->countdown_format == 'dd')
        <div id="cp-bar--countdown-days" style="margin-left:5px; margin-right:5px; float:left; padding:0; height:20px; width:50px; font-size:18px; font-weight:bold;
    ...
    @if ($bar->countdown_format != 'mm')
        <div id="cp-bar--countdown-hours" style="margin-right:5px; float:left; padding:0; height:20px; width:50px; font-size:18px; font-weight:bold;
            background-color: {{ $bar->countdown_background_color == '' ? 'transparent' : (strpos($bar->countdown_background_color, '#') === false ? '#' . $bar->countdown_background_color : $bar->countdown_background_color) }};">
            {{ $bar->countdown == 'evergreen' ? sprintf('%02d', $bar->countdown_hours) : sprintf('%02d', explode(':', $bar->countdown_end_time)[0]) }}
        </div>
    @endif
    <div id="cp-bar--countdown-minutes" style="margin-right:5px; float:left; padding:0; height:20px; width:50px; font-size:18px; font-weight:bold;
        background-color: {{ $bar->countdown_background_color == '' ? 'transparent' : (strpos($bar->countdown_background_color, '#') === false ? '#' . $bar->countdown_background_color : $bar->countdown_background_color) }};">
        {{ $bar->countdown == 'evergreen' ? sprintf('%02d', $bar->countdown_minutes) : sprintf('%02d', explode(':', $bar->countdown_end_time)[1]) }}
    </div>
    <div id="cp-bar--countdown-seconds" style="float:left; padding:0; height:20px; width:50px; font-size:18px; font-weight:bold;
        background-color: {{ $bar->countdown_background_color == '' ? 'transparent' : (strpos($bar->countdown_background_color, '#') === false ? '#' . $bar->countdown_background_color : $bar->countdown_background_color) }};">
        {{ date('s') }}
    </div>
</div>
