<div style="height:25px; width:100%; text-align:center;
    ...
    @if ($bar->countdown_format == 'dd')
        <div id="cp-bar--countdown-days" style="float:left;  padding: 0 0 0 12px; font-weight:bold; width: 55px; font-size:20px;">
    ...
    @if ($bar->countdown_format != 'mm')
        <div id="cp-bar--countdown-hours" style="float:left; padding: 0; font-weight:bold; width: 50px; font-size:20px;">
            {{ $bar->countdown == 'evergreen' ? sprintf('%02d', $bar->countdown_hours) : sprintf('%02d', explode(':', $bar->countdown_end_time)[0]) }}
        </div>
    @endif
    <div id="cp-bar--countdown-minutes" style="float:left; padding: 0; font-weight:bold; width: 50px; font-size:20px;">
        {{ $bar->countdown == 'evergreen' ? sprintf('%02d', $bar->countdown_minutes) : sprintf('%02d', explode(':', $bar->countdown_end_time)[1]) }}
    </div>
    <div id="cp-bar--countdown-seconds" style="float:left;  padding: 0 12px 0 0; font-weight:bold; width: 55px; font-size:20px;">
        {{ date('s') }}
    </div>
</div>
